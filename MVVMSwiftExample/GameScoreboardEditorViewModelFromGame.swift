...
class GameScoreboardEditorViewModelFromGame: NSObject, GameScoreboardEditorViewModel {
  ...
  fileprivate static func timeFormatted(totalMilliis: Int) -> String {
    let millis = totalMilliis % 1000 / 1000 // "/ 100" <- because we want only 1 digit
    let totalSeconds = totalMilliis / 1000
  ...
  fileprivate static func timeRemainingPretty(for game: Game) -> String {
    return timeFormatted(totalMilliis: Int(game.time * 1000))
  }
